...
def update_user(db: Session, user: schemas.User):
    db_user = db.query(models.Users).filter(models.User.id == user.id).first()
    db_user.name = user.name
    # db_user=update(models.Users).where(models.User.c.id==user.id).values(name=user.name)
    # db.execute(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def delete_user(db: Session, id: int):
    ...
    # db.execute(db_user)
    db_user = db.query(models.Users).filter(models.User.id == id).first()
    db.delete(db_user)
    db.commit()
    return db_user
