...
def get_deployment(db: Session, deployment_id: int):
    return db.query(models.Deployments).filter(models.Deployments.id == deployment_id).first()


def get_deployment_by_name(db: Session, name_deployment: str):
    return db.query(models.Deployments).filter(models.Deployments.name == name_deployment).first()


def create_deployment(db: Session, deployment: schemas.Deployment):
    db_deployment = models.Deployments(name=deployment.name,
...
def update_deployment(db: Session, deployment: schemas.Deployment, id:int):
    db_deployment = db.query(models.Deployments).filter(models.Deployments.id == id).first()
