...
def get_objects(db: Session, object: SQLModel, order_by, skip: int = 0, limit: int = 100):
...
def execute_query_all(db: Session, object: SQLModel, order_by, skip: int = 0, limit: int = 100):
    return get_objects(db=db, object=object, order_by=order_by, skip=skip, limit=limit).all()
