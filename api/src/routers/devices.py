...
from src.models.device import DeviceBase, DeviceMenu, ReadDevice
...
@router.get("/", response_model=List[ReadDevice])
def read_devices(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    devices = device.get_devices(db, skip=skip, limit=limit)
    return devices


@router.get("/{device_id}", response_model=ReadDevice)
...
@router.post("/", response_model=ReadDevice)
...
@router.put("/{device_id}", response_model=ReadDevice)
